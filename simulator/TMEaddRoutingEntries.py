...
class TMEaddRoutingEntries(OpenMeasure):
	def __init__(self, tmfile, sw_route_file, flow_route_file, prefix_file, SDNSwitchList, Tao):
		...
		self.cstr = "SNMPLinkLoads + OFS RoutingEntries"

	def TMEaddRoutingEntries(self):
		### populate OF switch routing entries
		swH = self.OM.SDNSwitchRoutingEntries_multi()
		for sw in swH:
			print "OF Switch {0} has routing entries {1}".format(sw, len(swH[sw]))
