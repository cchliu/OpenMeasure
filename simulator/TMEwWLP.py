...
class TMEwWLP(OpenMeasure):
	...
	def set_recordsNum(self, recordsNum):
	...
	def TMEwWLP(self):
		...
		predictValue = [0 for fl in range(self.N)]
			...
			for fl in range(self.N):
				records = traffic_estm[fl][max(t_epoch-self.recordsNum, 0):t_epoch][::-1]
				predictValue[fl] = sum([math.exp(-1*idx) * k for idx, k in enumerate(records)])
				predictValue[fl] *= scale
			
			index = sorted(range(len(predictValue)), key = lambda k: predictValue[k], reverse=True)
